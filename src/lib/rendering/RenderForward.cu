...
__global__ __launch_bounds__(DEFAULT_BLOCK_SIZE_FAST_COLLECT) void CountAndCollectTiled(
    ...
    float* __restrict__ full_list_buffer_data_ptr, StaticDeviceTensor<float, 4> depth_buffer_gs)

{
    ...
    for (int point_id = grid.thread_rank(); point_id < point_cloud.Size(); point_id += grid.size())
    {
...

        {
            ...
            float depth_val_this_px = depth_buffer_gs(batch, 0, int(round(ip.y())), int(round(ip.x())));
            float gs_eps            = d_render_params.depth_accept;
            if ((z - gs_eps) > depth_val_this_px) continue;
        }
        // write depth
        ...
        else
        {
            ...
            if (point_size_opt > 1)
            {
                ...
                layer_higher = min(__float2int_ru(log2f(2 * point_size_opt)), num_layers - 1);
            }
            int layer = 0;
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK_MULTITHREADLOAD2)
    FastSortFusedNew(int batch,
                     ...
                     StaticDeviceTensor<int32_t, 2, int> layer_lengths, int num_layers_input)
{
    ...
    __shared__ ivec2 wh_per_layer[max_layers];
    __shared__ int max_elements_sum;
    {
        int w, h;
        if (thread_idx < num_layers_input)
        {
            if (train)
            {
                w = static_cast<int>(d_forward_params.neural_out[thread_idx].size(3));
                h = static_cast<int>(d_forward_params.neural_out[thread_idx].size(2));
            }
            else
            {
                w = static_cast<int>(d_forward_params.neural_out[thread_idx].size(2));
                h = static_cast<int>(d_forward_params.neural_out[thread_idx].size(1));
            }
            wh_per_layer[thread_idx] = ivec2(w, h);
        }
        __syncthreads();

        if (thread_idx == 0)
        {
            int max_el_summed = w * h;
            for (int i = 1; i < num_layers_input; ++i) max_el_summed += wh_per_layer[i].x() * wh_per_layer[i].y();
            max_elements_sum = max_el_summed;
        }
        __syncthreads();
    }
    ...
    auto get_next_list = [&ticket, &glob_atomic_c, &layer, &gx, &gy, &num_layers_input, &length, &local_mem_warp,
                          &own_index_in_warp, &leader_length, &loop_run, &batch]
    {
        ...
        if (own_index_in_warp % WARP_LEADER_STRIDE == 0)
        {
            ...
            do
            {
                ...
                if (ticket < max_elements_sum)
                {
                    ...
                    ll_length = d_bilinear_alpha_params.per_pixel_list_lengths[layer](batch, gy, gx);
            ...
            bool all_failed = __all_sync(leader_mask, ticket >= max_elements_sum);
    ...
    while (loop_run)  //(ticket < max_elements_realigned)
    {
        ...
        // for (; leader_threads < WARP_SIZE / 2; leader_threads += WARP_LEADER_STRIDE)
        {
            ...
            auto get_element_addresses = [&own_index_in_warp, &threads_in_use, &ticket, &length, &batch, &leader_length,
                                          &start_offset, &layer_lengths,
                                          &num_layers_input](int layer_id, int gy_id, int gx_id)
            {
                if (own_index_in_warp == threads_in_use)
                {
                    if (ticket >= max_elements_sum || layer_id >= num_layers_input ||
                        gx_id >= wh_per_layer[layer_id].x() || gy_id >= wh_per_layer[layer_id].y())
                    {
                        length = 0;
                    }
                    else
                                      ...
                                      torch::Tensor background_color, CUDA::CudaTimerSystem* timer_system)
{
    ...
    // torch::Tensor non_zero_indices_t;
    {
        ...
        {
            layer_lengths =
                torch::zeros({1, info->num_layers}, torch::TensorOptions().dtype(torch::kInt32).device(torch::kCUDA));

            SAIGA_OPTIONAL_TIME_MEASURE("Scan Counts and division", info->timer_system);
            static void* d_temp_storage                = NULL;
            static size_t temp_storage_bytes_allocated = 0;
            static torch::Tensor tmp_tensor;
            for (int i = 0; i < info->num_layers; ++i)
            {
                auto& l = layers_cuda[i];

                {
                    SAIGA_OPTIONAL_TIME_MEASURE("Scan Counts " + std::to_string(i), info->timer_system);
                ...
                /*
                {
                    SAIGA_OPTIONAL_TIME_MEASURE("Div "+ std::to_string(i), info->timer_system);
                ...
                {
                    SAIGA_OPTIONAL_TIME_MEASURE("counz " + std::to_string(i), info->timer_system);

                    // auto max_scanned = l.scanned_counts.slice(0, batch, batch + 1)
                    //                       .slice(1, l.size.y() - 1, l.size.y())
                    //                       .slice(2, l.size.x() - 1, l.size.x());
                    // auto last_elem = l.per_pixel_list_lengths.slice(0, batch, batch + 1)
                    //                     .slice(1, l.size.y() - 1, l.size.y())
                    //                     .slice(2, l.size.x() - 1, l.size.x());
                    // PrintTensorInfo(max_scanned);
                    // PrintTensorInfo(last_elem);
                    auto max_scanned = l.scanned_counts.index({batch, l.size.y() - 1, l.size.x() - 1});
                    auto last_elem   = l.per_pixel_list_lengths.index({batch, l.size.y() - 1, l.size.x() - 1});

                    // PrintTensorInfo(max_scanned);
                    // PrintTensorInfo(last_elem);
                    num_elements += max_scanned + last_elem;

                    // write out list length accumulated, if not last element
                    if (i != (info->num_layers - 1))
                    {
                        //[1,1]
                        auto prev_len = layer_lengths.index({0, i});
                        layer_lengths.index_put_({0, i + 1}, max_scanned + last_elem + prev_len);
                        // layer_lengths.slice(0,0,1).slice(1,i+1,i+2)=max_scanned + last_elem + prev_len;
        ...
        }
        {
            ...
            //                     .clone()
            //                     .cuda();
            layer_lengths  = layer_lengths.contiguous().clone();
            int n_elements = num_elements.item<int>();
            ...
            full_list_buffer =
                torch::empty({1, n_elements, 1}, torch::TensorOptions().device(torch::kCUDA).dtype(torch::kFloat64));
...

    {
            ...
            switch (info->params.num_texture_channels)
            {
                case 3:
                {
                    FastSortFusedNew<3, 16, true><<<dim3(bx, by, 1), dim3(THREAD_DIMX_PER_BLOCK_MULTITHREADLOAD2,
                        ...
                        temp_full_buffer.data_ptr<float>(), background, atomic2, layer_lengths, info->num_layers);
                    break;
                }
                case 4:
                {
        ...
        {
            // std::cout << "p" << TensorInfo() << std::endl;
            switch (info->params.num_texture_channels)
            {
                case 3:
                {
