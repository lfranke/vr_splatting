                                        ...
                                        CUDA::CudaTimerSystem* timer_system)
{
    ...
    if (params.enable_exposure)
    {
        SAIGA_OPTIONAL_TIME_MEASURE("Exposure", timer_system);
    ...
    if (params.enable_white_balance)
    {
        SAIGA_OPTIONAL_TIME_MEASURE("White Balance", timer_system);
    ...
    if (params.enable_vignette)
    {
        SAIGA_OPTIONAL_TIME_MEASURE("Vignette", timer_system);
    ...
    if (camera_response)
    {
        SAIGA_OPTIONAL_TIME_MEASURE("Response", timer_system);
...
std::vector<vec3> NeuralCameraImpl::DownloadWhitebalance()
{
    auto ex_cpu = white_balance_values.cpu();
    std::vector<vec3> res;
    for (int i = 0; i < ex_cpu.size(0); ++i)
    {
        vec3 wb;
        wb.x() = ex_cpu[i][0][0][0].item().toFloat();
        wb.y() = ex_cpu[i][1][0][0].item().toFloat();
        wb.z() = ex_cpu[i][2][0][0].item().toFloat();
        res.push_back(wb);
    }
    return res;
}
void NeuralCameraImpl::InterpolateFromNeighbors(std::vector<int> indices)
{
    if (exposures_values.defined())
    {
        std::cout << "interpolating exposure for " << indices.size() << " images" << std::endl;

        std::vector<float> exp = DownloadExposure();
        int n                  = exp.size();
        // loop to interpolate if test indices are nearby
        for (int k = 0; k < 10; ++k)
        {
            for (int i : indices)
            {
                // wrap around indices
                int index_before = (i - 1) >= 0 ? i - 1 : n - 1;
                int index_after  = (i + 1) < n ? i + 1 : 0;


                exp[i] = 0.5f * (exp[index_before] + exp[index_after]);
            }
        }
    ...
    if (white_balance_values.defined())
    {
        std::cout << "interpolating wb for " << indices.size() << " images" << std::endl;

        std::vector<vec3> wbs = DownloadWhitebalance();
        int n                 = wbs.size();
        ...
        for (int k = 0; k < 10; ++k)
        {
            for (int i : indices)
            {
                ...
                wbs[i] = 0.5f * (wbs[index_before] + wbs[index_after]);
            }
        }

        torch::Tensor new_wb =
            torch::from_blob(wbs.data(), {(long)wbs.size(), 3L, 1L, 1L}, torch::TensorOptions().dtype(torch::kFloat32))
                .clone()
                .to(white_balance_values.options());

        torch::NoGradGuard ngg;
        white_balance_values.set_(new_wb);
    }
}
