...
struct LossResult
{
    ...
    LossResult& operator+=(const LossResult& other)
    {
        ...
        loss_similarity += other.loss_similarity;
    ...
    void AppendToFile(const std::string& file, int epoch)
    {
        ...
        tab << epoch << loss_vgg << loss_lpips << loss_l1 << loss_psnr << loss_ssim << loss_similarity
            << loss_float_param << count;
    }

    void Print()
    {
        console << "Param " << loss_float_param << " VGG " << loss_vgg << " L1 " << loss_l1 << " MSE " << loss_mse
                << " PSNR " << loss_psnr << " SSIM " << loss_ssim << " LPIPS " << loss_lpips << " similarity "
                << loss_similarity << " count " << count << std::endl;
    }
};
...
struct ForwardResult
{
    ...
    torch::Tensor visibility_filter;
    torch::Tensor radii;
    bool gaussians;
...
class NeuralPipeline
{
   public:
    ...
    CombineGSNPSettings combineparams[2];
