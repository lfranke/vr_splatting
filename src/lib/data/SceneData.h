...
struct ReducedImageInfo
{
    ...
    //gs
    bool own_proj_mat = false;
    Matrix<float, 4, 4> proj_mat;

    // fx,fy: zoom; cx,cy: translation; s=0;
    IntrinsicsPinholef crop_transform;
    Matrix<float, 2, 2> crop_rotation;

    void print()
    {
        std::cout << "ReducedImageInfo: " << std::endl;
        std::cout << "\t w: " << w << "\t h: " << h << "\t image_index: " << image_index
            << "\t camera_index: " << camera_index << "\t camera_model_type: " << (int)camera_model_type
            << "\t crop_transform: " << crop_transform << "\t crop_rotation: " << crop_rotation << std::endl;
    }
};
...
struct SceneDatasetParams : public ParamsBase
{
    ...
    void Params(ParamIterator* it)
    {
        ...
        SAIGA_PARAM_LIST(aabb_min, ' ');
        SAIGA_PARAM_LIST(aabb_max, ' ');
        ...
        aabb = AABB(vec3(aabb_min[0], aabb_min[1], aabb_min[2]), vec3(aabb_max[0], aabb_max[1], aabb_max[2]));
    }
    ...
    float point_factor = 0.0001;

    std::vector<double> aabb_min = {0.0, 0.0, 0.0};
    std::vector<double> aabb_max = {0.0, 0.0, 0.0};
    AABB aabb;
...
struct SceneCameraParams : public ParamsBase
{
    ...
    vec3 ImageToNormalized(vec2 ip, float z);
};

class SceneData
{
public:
    SceneData(std::string scene_path, bool full_preprocess = true);
    ...
    std::vector<SceneCameraParams> extra_eval_cameras;
    ...
    std::vector<::FrameData> extra_eval_frames;
    ...
    TemplatedImage<ucvec3> CPURenderFrameBilinear(int id, float scale);
    ...
    static void DuplicatePoints(Saiga::UnifiedMesh& point_cloud, int factor, float di);
    void DuplicatePointsRand(int factor, float dis);
    static void DuplicatePointsRand(Saiga::UnifiedMesh& point_cloud, int factor, float di = 0.5,
                                    bool pointgrow                                        = false);

    void AddRandomPoints(int num_add_points, AABB custom_bounding_box = AABB());

    static void AddRandomPoints(Saiga::UnifiedMesh& point_cloud, float fac = 0.1f);

    static void AddPoints(Saiga::UnifiedMesh& point_cloud, std::vector<vec3> positions, std::vector<vec3> normals);

    // after downsample
    // ~ n/factor points remain
    void DownsamplePoints(float factor);

    static void RemovePointsInCloseArea(Saiga::UnifiedMesh& point_cloud, std::vector<FrameData>& frames, float dis_mm);

    void RemovePointsInCloseArea(float dis_mm);

    void PointDistanceToCamera();
    static void PointDistanceToCamera(Saiga::UnifiedMesh& point_cloud, std::vector<FrameData>& frames);
    ...
    static void ComputeRadius(Saiga::UnifiedMesh& point_cloud, int n = 4);
