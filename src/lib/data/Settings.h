...
struct RenderParams : public ParamsBase
{
    ...
    bool render_trips        = true;
    bool render_foveated     = true;
    bool render_gaussian     = true;
    std::string stp_config   = "stp_config.json";
    bool complex_fovea_merge = true;
    bool opacity_decay       = true;
    ...
    void Params(ParamIterator* it)
    {
        ...
        SAIGA_PARAM(render_trips);
        SAIGA_PARAM(render_foveated);
        SAIGA_PARAM(render_gaussian);
        SAIGA_PARAM(stp_config);
        SAIGA_PARAM(complex_fovea_merge);
        SAIGA_PARAM(opacity_decay);
...
struct OptimizerParams : public ParamsBase
{
    ...
    void Params(ParamIterator* it)
    {
        ...
        SAIGA_PARAM(reset_descriptors_interval);

        SAIGA_PARAM(number_of_descriptor_resets);
    ...
    int reset_descriptors_interval  = -1;
    int number_of_descriptor_resets = -1;
    // structure
...
struct PipelineParams : public ParamsBase
{
    ...
    void Params(ParamIterator* it)
    {
        ...
        SAIGA_PARAM(add_gs_to_network);
        SAIGA_PARAM(gaussian_densification_abs);
        SAIGA_PARAM(gs_sh_degrees);
    }
    ...
    bool gaussian_densification_abs = true;
    // bool add_depth_to_network = false;
