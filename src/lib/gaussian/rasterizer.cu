...
TORCH_LIBRARY(gs_raster_settings_for_custom_modulefunc, m)
{
    std::cout << "register settings" << std::endl;
    m.class_<GaussianRasterizationSettings>("GaussianRasterizationSettings").def(torch::init());
}
                                                 ...
                                                 GaussianRasterizationSettings raster_settings)
{
    ...
    auto scale_modifier = torch::full({1}, raster_settings.scale_modifier, device);
    ...
    torch::intrusive_ptr<GaussianRasterizationSettings> render_data =
        torch::make_intrusive<GaussianRasterizationSettings>(raster_settings);

    // if (raster_settings.viewer_only)
    //{
    //     return _RasterizeGaussians::forward(nullptr, means3D, means2D, sh, colors_precomp, opacities, scales,
    //     rotations,
    //                                         cov3Ds_precomp, image_height, image_width, tanfovx, tanfovy,
    //                                         raster_settings.bg, scale_modifier, raster_settings.viewmatrix,
    //                                         raster_settings.projmatrix, raster_settings.inv_viewprojmatrix,
    //                                         sh_degree, raster_settings.camera_center, prefiltered,
    //                                         torch::IValue(render_data));
    // }
    return _RasterizeGaussians::apply(means3D, means2D, sh, colors_precomp, opacities, scales, rotations,
                                      cov3Ds_precomp, image_height, image_width, tanfovx, tanfovy, raster_settings.bg,
                                      scale_modifier, raster_settings.viewmatrix, raster_settings.projmatrix,
                                      raster_settings.inv_viewprojmatrix, sh_degree, raster_settings.camera_center,
                                      prefiltered, torch::IValue(render_data));
}
