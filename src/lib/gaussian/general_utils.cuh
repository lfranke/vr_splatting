 ...
 * the final learning rate, the number of steps to delay the decay, the delay multiplier, and the maximum number of
 * steps.
 *
...
struct Expon_lr_func
{
    ...
    float lr_delay_mult;
    int64_t max_steps;
    Expon_lr_func(float lr_init = 0.f, float lr_final = 1.f, float lr_delay_mult = 1.f, int64_t max_steps = 1000000,
                  float lr_delay_steps = 0.f)
        : lr_init(lr_init),
          lr_final(lr_final),
    ...
    float operator()(int64_t step) const
    {
        ...
        {
            delay_rate = lr_delay_mult + (1 - lr_delay_mult) *
                                             std::sin(0.5 * M_PI * std::clamp((float)step / lr_delay_steps, 0.f, 1.f));
        }
        else
        {
            delay_rate = 1.0;
...
void SaveTensorToImage(at::Tensor tensor, std::string path);

// double output to log files class
// from: https://stackoverflow.com/a/32481585
class CTee
{
   public:
    // Use ostream &s2 if you want to duplicate to an ostream, pass other
    // stuff you need if you have other logging mechanisms.
    CTee(std::ostream& s1, std::ostream& s2);
    ~CTee();
   ...
   private:
    ...
    class CTeeBuf : public std::streambuf
    {
       public:
        ...
        CTeeBuf(std::streambuf* sb1, std::streambuf* sb2);
...
void set_border_to_zero(torch::Tensor& tensor, int width);