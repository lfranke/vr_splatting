...
// #include "cuda_rasterizer/STP_settings.h"

class GaussianRasterizationSettings : public torch::CustomClassHolder
{
   public:
    int image_height;
    int image_width;
    ...
    bool prefiltered;
    CudaRasterizer::SplattingSettings splatting_args;
    bool render_depth;
    bool debug;
    bool viewer_only;
};

torch::autograd::tensor_list rasterize_gaussians(torch::Tensor means3D, torch::Tensor means2D, torch::Tensor sh,
...
class _RasterizeGaussians : public torch::autograd::Function<_RasterizeGaussians>
{
   public:
    static torch::autograd::tensor_list forward(
        torch::autograd::AutogradContext* ctx, torch::Tensor means3D, torch::Tensor means2D, torch::Tensor sh,
        torch::Tensor colors_precomp, torch::Tensor opacities, torch::Tensor scales, torch::Tensor rotations,
        torch::Tensor cov3Ds_precomp, torch::Tensor image_height, torch::Tensor image_width, torch::Tensor tanfovx,
        torch::Tensor tanfovy, torch::Tensor bg, torch::Tensor scale_modifier, torch::Tensor viewmatrix,
        torch::Tensor projmatrix, torch::Tensor inv_viewprojmatrix, torch::Tensor sh_degree,
        torch::Tensor camera_center, torch::Tensor prefiltered, torch::IValue settings)
    {
        ...
        //  std::cout << scale_modifier_val << std::endl;
        GaussianRasterizationSettings* render_settings = settings.toCustomClass<GaussianRasterizationSettings>().get();

        CudaRasterizer::SplattingSettings spl_settings = render_settings->splatting_args;
        ...
        auto [num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer] = RasterizeGaussiansCUDA(
            bg, means3D, colors_precomp, opacities, scales, rotations, scale_modifier_val, cov3Ds_precomp, viewmatrix,
            projmatrix, inv_viewprojmatrix, tanfovx_val, tanfovy_val, image_height_val, image_width_val, sh,
            sh_degree_val, camera_center, prefiltered_val, spl_settings, false, false);

        if (ctx)
        {
            ctx->save_for_backward({colors_precomp, means3D, opacities, scales, rotations, cov3Ds_precomp, radii, color,
            ...
            ctx->saved_data["inv_viewprojmatrix"] = inv_viewprojmatrix;
                                                 ...
                                                 torch::autograd::tensor_list grad_outputs)
    {
        ...
        auto opacities      = saved[2];
        auto scales         = saved[3];
        auto rotations      = saved[4];
        auto cov3Ds_precomp = saved[5];
        auto radii          = saved[6];
        auto pixel_colors   = saved[7];
        auto sh             = saved[8];
        ...
        auto [grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_sh,
              grad_scales, grad_rotations] =
            RasterizeGaussiansBackwardCUDA(
                ctx->saved_data["background"].to<torch::Tensor>(), means3D, radii, opacities, colors_precomp, scales,
                rotations, ctx->saved_data["scale_modifier"].to<float>(), cov3Ds_precomp,
                ctx->saved_data["viewmatrix"].to<torch::Tensor>(), ctx->saved_data["projmatrix"].to<torch::Tensor>(),
                ctx->saved_data["inv_viewprojmatrix"].to<torch::Tensor>(), ctx->saved_data["tanfovx"].to<float>(),
                ctx->saved_data["tanfovy"].to<float>(), pixel_colors, grad_out_color, sh,
                ctx->saved_data["sh_degree"].to<int>(), ctx->saved_data["camera_center"].to<torch::Tensor>(),
                geomBuffer, num_rendered, binningBuffer, imgBuffer, spl_settings, false);

#ifdef WRITE_TEST_DATA
        saveFunctionData("rasterize_backward_test_data.dat", grad_means2D, grad_colors_precomp, grad_opacities,
                         grad_means3D, grad_cov3Ds_precomp, grad_sh, grad_scales, grad_rotations, background_copy,
                         means3D_copy, radii_copy, colors_precomp_copy, scales_copy, rotations_copy,
                         scale_modifier_copy, cov3Ds_precomp_copy, viewmatrix_copy, projmatrix_copy, tanfovx_copy,
                         tanfovy_copy, grad_out_color_copy, sh_copy, sh_degree_copy, camera_center_copy,
                         geomBuffer_copy, num_rendered_copy, binningBuffer_copy, imgBuffer_copy);
#endif
        // return gradients for all inputs, 22 in total. :D
...
{
   public:
    GaussianRasterizer(GaussianRasterizationSettings raster_settings, Saiga::CUDA::CudaTimerSystem* timer_sys = nullptr)
        : raster_settings_(raster_settings), timer_system(timer_sys)
    {
    }
    GaussianRasterizer() {}
    ...
    std::tuple<torch::Tensor, torch::Tensor> forward(torch::Tensor means3D, torch::Tensor means2D,
                                                     torch::Tensor opacities, torch::Tensor shs = torch::Tensor(),
                                                     ...
                                                     torch::Tensor cov3D_precomp  = torch::Tensor())
    {
        ...
        auto result = rasterize_gaussians(means3D, means2D, shs, colors_precomp, opacities, scales, rotations,
                                          cov3D_precomp, raster_settings_);

        return {result[0], result[1]};
    }

   private:
    Saiga::CUDA::CudaTimerSystem* timer_system;
