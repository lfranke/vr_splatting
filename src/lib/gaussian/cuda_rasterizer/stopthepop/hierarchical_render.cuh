...
#pragma unroll
    for (int32_t i = NUM / 2; i > 0; i /= 2)
    {
        if (keys[s1 + i] < store_key) s1 += i;
    }
    // move one ahead
    s1 += 1;
    if (keys[0] >= store_key) s1 = 0;
    // how many threads of my group are ahead of me
    s1 += cg.thread_rank();
...
#pragma unroll
        ...
        {
            if (key < keys[access_function(loc - i)]) loc -= i;
        }
    }
    loc += rank;
...
#pragma unroll
    for (int i = 1; i < N; ++i)
    {
        ...
        if (other_key < key || (other_key == key && other_rank < rank))
        {
            ++count;
        }
    }
...
__device__ void sortGaussiansRayHierarchicaEvaluation(
    const uint2* __restrict__ ranges, const uint32_t* __restrict__ point_list, int W, int H,
    const float2* __restrict__ points_xy_image, const float4* __restrict__ cov3Ds_inv,
    const float* __restrict__ projmatrix_inv, const float3* __restrict__ cam_pos,
    const float4* __restrict__ conic_opacity, DebugVisualization debugType, PF&& prep_function, SF&& store_function,
    BF&& blend_function, FF&& fin_function)
{
#if (DEBUG_HIERARCHICAL & 0x100) != 0
    // if (blockIdx.x != 7 || blockIdx.y != 7)
    //	return;
    constexpr uint2 target = {426, 55};
    if (blockIdx.x != target.x / 16 || blockIdx.y != target.y / 16) return;
    uint2 rem = {target.x % 16, target.y % 16};
    if (cg::this_thread_block().thread_rank() / 32 != rem.x / 8 + (rem.y / 4) * 2) return;
    ...
    [[maybe_unused]] auto mid_access    = [&](uint32_t offset) { return (mid_front + offset) % MidSortWindow; };
    ...
    if (block.thread_index().x < 5)
    {
        ...
        printf("group dir %d - %d %d %d  - pix %f %f dir %f %f %f\n", warp.thread_rank(), block.thread_index().y,
               block.thread_index().z, block.thread_index().x, pos.x, pos.y, dir.x, dir.y, dir.z);
#endif
    }
    ...
    const float3 viewdir =
        computeViewRay(inverse_vp, campos, float2{static_cast<float>(pixpos.x), static_cast<float>(pixpos.y)}, W, H);
#if (DEBUG_HIERARCHICAL & 0x2F) != 0 && (DEBUG_HIERARCHICAL & 0x100) != 0
    printf("own dir %d - %d %d %d  - pix %d %d dir %f %f %f\n", warp.thread_rank(), block.thread_index().y,
           block.thread_index().z, block.thread_index().x, pixpos.x, pixpos.y, viewdir.x, viewdir.y, viewdir.z);
#endif
    // setup helpers
...
#    endif
            printf("%d - %d %d - blending: %f %d %f (%d %d %d)\n", warp.thread_rank(), pixpos.x, pixpos.y,
                   head_depths[0], head_ids[0], head_stores[0], (fill_counters & FillHeadMask) / FillHeadOne,
                   (fill_counters & FillMidMask) / FillMidOne, (fill_counters & FillTailMask) / FillTailOne);
#endif
    ...
    auto front4OneFromMid = [&](bool checkvalid)
    {
        if (head_group.any(active))
        {
            ...
            if constexpr (MidSortWindow == 8)
            {
                load_id = mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                                 [warp.thread_rank() % 4];
            }
            else
            {
            ...
            for (int inner = 0; inner < 4; ++inner)
            {
                ...
                {
                    coll_id =
                        mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4][inner];
                }
                else
                {
                    coll_id = mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                                     [mid_access(inner)];
                }

                // every thread has the same id so this is safe
                if (checkvalid && coll_id == -1) continue;
...
#pragma unroll
                ...
                printf("%d - %d %d - count: %d - sorted: %f %d - %f %d - %f %d - %f %d\n", warp.thread_rank(), pixpos.x,
                       pixpos.y, fill_counters, head_depths[0], head_ids[0], head_depths[1], head_ids[1],
                       head_depths[2], head_ids[2], head_depths[3], head_ids[3]);
#endif
            }
        }
        if constexpr (MidSortWindow != 8)
    ...
    auto pushPullThroughMid = [&](bool checkvalid)
    {
        ...
        printf("%d - %d %d - mid loading %d\n", warp.thread_rank(), block.thread_index().y, block.thread_index().z,
               load_id);
#endif
        ...
        for (int mid = 0; mid < 4; ++mid)
        {
            ...
            if (checkvalid && (fill_counters & FillTailMask) == 0) break;
            ...
            float depth = depthAlongRay(
                halfwarp.shfl(tail_depth_info[0], tid), halfwarp.shfl(tail_depth_info[1], tid),
            ...
            if constexpr (MidSortWindow == 8)
            {
                ...
                coll_id = mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                                 [warp.thread_rank() % 4];
                depth = mid_depths[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                                  [warp.thread_rank() % 4];

#if (DEBUG_HIERARCHICAL & 0x2) != 0
                printf("%d - %d %d %d - mid local %d sort %d %f \n", warp.thread_rank(), block.thread_index().y,
                       block.thread_index().z, block.thread_index().x / 4, warp.thread_rank() % 4, coll_id, depth);
#endif
                ...
                if (mid != 0 || ((fill_counters & FillMidMask) > 4 * FillMidOne))
                {
                    ...
                    mergeSortRegToSmem<4>(
                        ...
                        mid_depths[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4],
                        mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4], depth,
                        coll_id);
                    head_group.sync();
#if (DEBUG_HIERARCHICAL & 0x2) != 0
                    printf("%d - %d %d %d - sorted into mid %d:  %d %f \n", warp.thread_rank(), block.thread_index().y,
                           block.thread_index().z, block.thread_index().x / 4, warp.thread_rank() % 4,
                           mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                                  [warp.thread_rank() % 4],
                           mid_depths[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                                     [warp.thread_rank() % 4]);
                    printf("%d - %d %d %d - sorted into mid %d:  %d %f \n", warp.thread_rank(), block.thread_index().y,
                           block.thread_index().z, block.thread_index().x / 4, 4 + warp.thread_rank() % 4,
                           mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
            ...
            else
            {
                ...
                mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4][sort_mid_offset] =
                    coll_id;
                mid_depths[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                          [sort_mid_offset] = depth;

                head_group.sync();

                coll_id =
                    mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4][my_mid_offset];
                depth = mid_depths[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                                  [my_mid_offset];
                ...
                mergeSortInto<4, MidSortWindow - 4>(
                    head_group, warp.thread_rank() % 4, depth, coll_id,
                    mid_depths[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4],
                    mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4], mid_access);
                ...
                for (int j = 0; j < MidSortWindow; j += 4)
                {
                    ...
                    printf(
                        ...
                        block.thread_index().y, block.thread_index().z, block.thread_index().x / 4,
    ...
    for (int progress = range.x; progress < range.y; progress += WARP_SIZE)
    {
            ...
            if (load_id != -1)
            {
                ...
                if (CULL_ALPHA)
                {
                    ...
                    const glm::vec2 tail_rect_min = {
                        ...
                        static_cast<float>(block.group_index().y * BLOCK_Y + 4 * block.thread_index().z)};
                    const glm::vec2 tail_rect_max = {tail_rect_min.x + 3.0f, tail_rect_min.y + 3.0f};

                    glm::vec2 max_pos;
                    float power = max_contrib_power_rect_gaussian_float<3, 3>(in_conic_opacity, in_point_xy,
                                                                              tail_rect_min, tail_rect_max, max_pos);

                    float alpha = min(0.99f, in_conic_opacity.w * exp(-power));
                    if (alpha < 1.0f / 255.0f) halfs_culled_mask |= (0x1U << half);
                }
            }
        }
        ...
        for (int half = 0; half < 2; ++half)
        {
            ...
            if (load_id != -1)
            {
                ...
                {
                    depth = depthAlongRay(in_depth_info[0], in_depth_info[1], in_depth_info[2],
...
#if (DEBUG_HIERARCHICAL & 0x1) != 0
            printf("(%d) %d - %d %d %d - %d : %f\n", half, warp.thread_rank(), xid, block.thread_index().z, 0, load_id,
                   depth);
#endif
        }
        // local sort the 32 elements with half warp from shared memory
        batcherSort<32>(halfwarp, tail_depths[block.thread_index().y][block.thread_index().z] + 32,
                        tail_ids[block.thread_index().y][block.thread_index().z] + 32);
        // sync comes through shfl below

#if (DEBUG_HIERARCHICAL & 0x1) != 0
        printf("batcher sort %d/%d: %f %d\n", block.thread_index().y, halfwarp.thread_rank(),
               tail_depths[block.thread_index().y][block.thread_index().z][32 + halfwarp.thread_rank()],
               tail_ids[block.thread_index().y][block.thread_index().z][32 + halfwarp.thread_rank()]);
        printf("batcher sort %d/%d: %f %d\n", block.thread_index().y, 16 + halfwarp.thread_rank(),
               tail_depths[block.thread_index().y][block.thread_index().z][32 + 16 + halfwarp.thread_rank()],
               tail_ids[block.thread_index().y][block.thread_index().z][32 + 16 + halfwarp.thread_rank()]);
#endif
        ...
        for (int half = 0; half < 2; ++half)
        {
            ...
            if ((warp.shfl(fill_counters, half * 16) & FillTailMask) != 0)
            {
                ...
                if (half == warp.thread_rank() / 16) fill_counters += count_valid * FillTailOne;
                mergeSortRegToSmem<32>(warp, d, id, d, id, k, v);

#if (DEBUG_HIERARCHICAL & 0x1) != 0
                warp.sync();
                printf("merge of %d (%d) sort %d: %f %d\n", xid, (fill_counters & FillTailMask) / FillTailOne,
                       warp.thread_rank(), d[warp.thread_rank()], id[warp.thread_rank()]);
                printf("merge of %d (%d) sort %d: %f %d\n", xid, (fill_counters & FillTailMask) / FillTailOne,
                       32 + warp.thread_rank(), d[32 + warp.thread_rank()], id[32 + warp.thread_rank()]);
#endif
            }
            else
            {
    ...
    printf("%d - %d %d %d - draining tail with %d %d %d\n", warp.thread_rank(), block.thread_index().y,
           block.thread_index().z, block.thread_index().x, (fill_counters & FillHeadMask) / FillHeadOne,
           (fill_counters & FillMidMask) / FillMidOne, (fill_counters & FillTailMask) / FillTailOne);
#endif
        ...
        if ((fill_counters & FillTailMask) != 0)
        {
            for (int half = 0; half < 2; ++half)
            {
                ...
                printf("%d - %d %d %d - pulled from mid %d  with %d %d %d\n", warp.thread_rank(),
                       block.thread_index().y, block.thread_index().z, block.thread_index().x, half,
                       (fill_counters & FillHeadMask) / FillHeadOne, (fill_counters & FillMidMask) / FillMidOne,
                       (fill_counters & FillTailMask) / FillTailOne);
#endif
                if ((half == 0) && (fill_counters & FillTailMask) == 0) break;
        ...
        if (warp.any(active))
        {
            if constexpr (MidSortWindow == 8)
            {
                if ((fill_counters & FillMidMask) != 0)
                {
                    ...
                    mid_ids[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                           [warp.thread_rank() % 4] = mid_ids[block.thread_index().y][block.thread_index().z]
                                                             [block.thread_index().x / 4][4 + warp.thread_rank() % 4];
                    mid_depths[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                              [warp.thread_rank() % 4] =
                                  mid_depths[block.thread_index().y][block.thread_index().z][block.thread_index().x / 4]
                                            [4 + warp.thread_rank() % 4];
                    ...
                    printf("%d - %d %d %d - pulled took 4 from mid %d %d %d\n", warp.thread_rank(),
                           block.thread_index().y, block.thread_index().z, block.thread_index().x,
                           (fill_counters & FillHeadMask) / FillHeadOne, (fill_counters & FillMidMask) / FillMidOne,
                           (fill_counters & FillTailMask) / FillTailOne);
#endif
                }
            ...
            else
            {
                ...
                while ((fill_counters & FillMidMask) != 0)
                {
                    ...
                    printf("%d - %d %d %d - pulled (%d) took 4 from mid %d %d %d\n", warp.thread_rank(),
                           block.thread_index().y, block.thread_index().z, block.thread_index().x, deb_counter,
                           (fill_counters & FillHeadMask) / FillHeadOne, (fill_counters & FillMidMask) / FillMidOne,
                           (fill_counters & FillTailMask) / FillTailOne);
                    ++deb_counter;
#endif
...
__global__ void __launch_bounds__(16 * 16)
    sortGaussiansRayHierarchicalCUDA_forward(const uint2* __restrict__ ranges, const uint32_t* __restrict__ point_list,
                                             ...
                                             const float3* __restrict__ cam_pos, const float* __restrict__ features,
                                             ...
                                             DebugVisualization debugType, float* __restrict__ out_color,
                                             bool half_float_result)
{
    // int num_blends = 0;
    struct BlendDataRaw
    {
    ...
    auto store_function = [](const uint2&, int coll_id, float G, float alpha, float depth) { return alpha; };
    auto blend_function =
        [&](const uint2& pixpos, BlendData& blend_data, int id, float alpha, float depth, DebugVisualization debugType)
    {
        ...
        for (int ch = 0; ch < CHANNELS; ch++) blend_data.C[ch] += features[id * CHANNELS + ch] * alpha * blend_data.T;
        // blend_data.accumDepth += depth * min(alpha + 0.2f, 1.f) * blend_data.T;
        blend_data.accumDepth += depth * alpha * blend_data.T;

        // ++num_blends;
        if constexpr (ENABLE_DEBUG_VIZ)
        {
            accumSortingErrorDepth(debugType, blend_data.currentDepth, depth, alpha, blend_data.T,
                                   blend_data.accumDepth, blend_data.errorCounter);
        }
    ...
    auto fin_function = [&](const uint2& pixpos, BlendData& blend_data, DebugVisualization debugType, int range)
    {
        ...
        if constexpr (!ENABLE_DEBUG_VIZ)
        {
            if (half_float_result)
            {
                __half* out_col_casted = (__half*)out_color;
                for (int ch = 0; ch < CHANNELS; ch++)
                    out_col_casted[ch * H * W + pix_id] = __half(blend_data.C[ch] + blend_data.T * bg_color[ch]);
                out_col_casted[CHANNELS * H * W + pix_id] = __half(blend_data.accumDepth);
            }
            else
            {
                for (int ch = 0; ch < CHANNELS; ch++)
                    out_color[ch * H * W + pix_id] = blend_data.C[ch] + blend_data.T * bg_color[ch];
                out_color[CHANNELS * H * W + pix_id] = blend_data.accumDepth;
            }
        }
        else
        {
            outputDebugVis(debugType, out_color, pix_id, blend_data.contributor, blend_data.T, blend_data.accumDepth,
                           blend_data.errorCounter, range, H, W);
        }
    };
...
__global__ void __launch_bounds__(16 * 16) sortGaussiansRayHierarchicalCUDA_backward(
    const uint2* __restrict__ ranges, const uint32_t* __restrict__ point_list, int W, int H,
    const float* __restrict__ bg_color, const float2* __restrict__ points_xy_image,
    const float4* __restrict__ cov3Ds_inv, const float* __restrict__ projmatrix_inv, const float3* __restrict__ cam_pos,
    const float4* __restrict__ conic_opacity, const float* __restrict__ colors, const float* __restrict__ final_Ts,
    const uint32_t* __restrict__ n_contrib, const float* __restrict__ pixel_colors,
    const float* __restrict__ dL_dpixels, float3* __restrict__ dL_dmean2D, float4* __restrict__ dL_dconic2D,
    float* __restrict__ dL_dopacity, float* __restrict__ dL_dcolors)
{
    ...
    auto store_function = [](const uint2&, int coll_id, float G, float alpha, float depth) { return G; };
    auto blend_function = [&](const uint2& pixpos, BlendData& blend_data, int global_id, float G, float depth,
                              DebugVisualization debugType)
    {
        ...
        for (int i = 0; i < CHANNELS; i++) bg_dot_dpixel += bg_color[i] * blend_data.dL_dpixel[i];
        ...
        atomicAdd(&dL_dmean2D[global_id].z, abs(dL_dG * dG_ddelx * ddelx_dx) + abs(dL_dG * dG_ddely * ddely_dy));
    ...
    sortGaussiansRayHierarchicaEvaluation<HEAD_WINDOW, MID_WINDOW, CULL_ALPHA>(
        ranges, point_list, W, H, points_xy_image, cov3Ds_inv, projmatrix_inv, cam_pos, conic_opacity,
        DebugVisualization::Disabled, prep_function, store_function, blend_function, fin_function);
}
