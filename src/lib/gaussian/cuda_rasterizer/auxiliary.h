...
__forceinline__ __device__ glm::vec3 pix2world(const glm::vec2 pix, const int W, const int H, glm::vec4 inverse_vp0,
                                               glm::vec4 inverse_vp1, glm::vec4 inverse_vp3)
{
...
__forceinline__ __device__ glm::vec3 pix2world(const glm::vec2 pix, const int W, const int H,
...
__forceinline__ __device__ void getRect(const float2 p, const float2 rect_extent, uint2& rect_min, uint2& rect_max,
                                        dim3 grid)
{
    rect_min = {min(grid.x, max((int)0, (int)floorf((p.x - rect_extent.x) / BLOCK_X))),
                min(grid.y, max((int)0, (int)floorf((p.y - rect_extent.y) / BLOCK_Y)))};
    rect_max = {min(grid.x, max((int)0, (int)ceilf((p.x + rect_extent.x) / BLOCK_X))),
...
__forceinline__ __device__ bool in_frustum(int idx, const glm::vec3 mean3D, const glm::mat4x3 viewmatrix,
                                           bool prefiltered, glm::vec3& p_view)
{
