...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos,
                                   const float* shs, const bool* clamped, const glm::vec3* dL_dcolor,
                                   glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
    ...
    if (deg > 0)
    {
        ...
        if (deg > 1)
        {
            ...
            dRGBdx +=
                SH_C2[0] * y * sh[4] + SH_C2[2] * 2.f * -x * sh[6] + SH_C2[3] * z * sh[7] + SH_C2[4] * 2.f * x * sh[8];
            dRGBdy +=
                SH_C2[0] * x * sh[4] + SH_C2[1] * z * sh[5] + SH_C2[2] * 2.f * -y * sh[6] + SH_C2[4] * 2.f * -y * sh[8];
...
__global__ void computeCov2DCUDA(int P, const bool proper_ewa_scaling, const float3* means, const int* radii,
                                 const float* cov3Ds, const float h_x, float h_y, const float tan_fovx, float tan_fovy,
                                 const float* view_matrix, const float* opacities, const float* dL_dconics,
                                 float* dL_dopacity, float3* dL_dmeans, float* dL_dcov)
{
    ...
    if (proper_ewa_scaling)
    {
        ...
        const float h_convolution_scaling =
            sqrt(max(0.000025f, det_cov_orig / det_cov_plus_h_cov));  // max for numerical stability
        ...
        const float d_inside_root           = (det_cov_orig / det_cov_plus_h_cov) <= 0.000025f
                                                  ? 0.f
    ...
    if (denom2inv != 0)
    {
        ...
        dL_dc_xx += denom2inv *
                    (-c_yy * c_yy * dL_dconic.x + 2 * c_xy * c_yy * dL_dconic.y + (denom - c_xx * c_yy) * dL_dconic.z);
        dL_dc_yy += denom2inv *
                    (-c_xx * c_xx * dL_dconic.z + 2 * c_xx * c_xy * dL_dconic.y + (denom - c_xx * c_yy) * dL_dconic.x);
        dL_dc_xy += denom2inv * 2 *
                    (c_xy * c_yy * dL_dconic.x - (denom + 2 * c_xy * c_xy) * dL_dconic.y + c_xx * c_xy * dL_dconic.z);

        // Gradients of loss L w.r.t. each 3D covariance matrix (Vrk) entry,
        // given gradients w.r.t. 2D covariance matrix (diagonal).
        // cov2D = transpose(T) * transpose(Vrk) * T;
        dL_dcov[6 * idx + 0] =
            (T[0][0] * T[0][0] * dL_dc_xx + T[0][0] * T[1][0] * dL_dc_xy + T[1][0] * T[1][0] * dL_dc_yy);
        dL_dcov[6 * idx + 3] =
            (T[0][1] * T[0][1] * dL_dc_xx + T[0][1] * T[1][1] * dL_dc_xy + T[1][1] * T[1][1] * dL_dc_yy);
        dL_dcov[6 * idx + 5] =
            (T[0][2] * T[0][2] * dL_dc_xx + T[0][2] * T[1][2] * dL_dc_xy + T[1][2] * T[1][2] * dL_dc_yy);

        // Gradients of loss L w.r.t. each 3D covariance matrix (Vrk) entry,
        // given gradients w.r.t. 2D covariance matrix (off-diagonal).
        // Off-diagonal elements appear twice --> double the gradient.
        // cov2D = transpose(T) * transpose(Vrk) * T;
        dL_dcov[6 * idx + 1] = 2 * T[0][0] * T[0][1] * dL_dc_xx + (T[0][0] * T[1][1] + T[0][1] * T[1][0]) * dL_dc_xy +
                               2 * T[1][0] * T[1][1] * dL_dc_yy;
        dL_dcov[6 * idx + 2] = 2 * T[0][0] * T[0][2] * dL_dc_xx + (T[0][0] * T[1][2] + T[0][2] * T[1][0]) * dL_dc_xy +
                               2 * T[1][0] * T[1][2] * dL_dc_yy;
        dL_dcov[6 * idx + 4] = 2 * T[0][2] * T[0][1] * dL_dc_xx + (T[0][1] * T[1][2] + T[0][2] * T[1][1]) * dL_dc_xy +
                             ...
                             glm::vec3* dL_dscales, glm::vec4* dL_drots)
{
    ...
    glm::mat3 dL_dSigma =
        glm::mat3(dL_dcov3D[0], 0.5f * dL_dcov3D[1], 0.5f * dL_dcov3D[2], 0.5f * dL_dcov3D[1], dL_dcov3D[3],
...
template <int C>
__global__ void preprocessCUDA(int P, int D, int M, const float3* means, const int* radii, const float* shs,
                               const bool* clamped, const glm::vec3* scales, const glm::vec4* rotations,
                               const float scale_modifier, const float* proj, const glm::vec3* campos,
                               const float3* dL_dmean2D, glm::vec3* dL_dmeans, float* dL_dcolor, float* dL_dcov3D,
...
void BACKWARD::preprocess(int P, int D, int M, const bool proper_ewa_scaling, const float3* means3D, const int* radii,
                          const float* shs, const bool* clamped, const float* opacities, const glm::vec3* scales,
                          const glm::vec4* rotations, const float scale_modifier, const float* cov3Ds,
                          const float* viewmatrix, const float* projmatrix, const float focal_x, float focal_y,
                          const float tan_fovx, float tan_fovy, const glm::vec3* campos, const float3* dL_dmean2D,
                          const float* dL_dconic, float* dL_dopacity, glm::vec3* dL_dmean3D, float* dL_dcolor,
                          float* dL_dcov3D, float* dL_dsh, glm::vec3* dL_dscale, glm::vec4* dL_drot)
{
    ...
    computeCov2DCUDA<<<(P + 255) / 256, 256>>>(P, proper_ewa_scaling, means3D, radii, cov3Ds, focal_x, focal_y,
                                               tan_fovx, tan_fovy, viewmatrix, opacities, dL_dconic, dL_dopacity,
    ...
    preprocessCUDA<NUM_CHANNELS>
        <<<(P + 255) / 256, 256>>>(P, D, M, (float3*)means3D, radii, shs, clamped, (glm::vec3*)scales,
                                   (glm::vec4*)rotations, scale_modifier, projmatrix, campos, (float3*)dL_dmean2D,
                                   (glm::vec3*)dL_dmean3D, dL_dcolor, dL_dcov3D, dL_dsh, dL_dscale, dL_drot);
}

void BACKWARD::render(const dim3 grid, const dim3 block, const uint2* ranges, SortSettings sort_settings,
                      CullingSettings culling_settings, const uint32_t* point_list, int W, int H, const float* bg_color,
                      const float2* means2D, const float4* cov3D_inv, const float* projmatrix_inv,
                      const glm::vec3* cam_pos, const float4* conic_opacity, const float* colors, const float* final_Ts,
                      const uint32_t* n_contrib, const float* pixel_colors, const float* dL_dpixels, float3* dL_dmean2D,
                      float4* dL_dconic2D, float* dL_dopacity, float* dL_dcolors)
{
#define CALL_KBUFFER(WINDOW)                                                                                   \
    renderkBufferBackwardCUDA<NUM_CHANNELS, WINDOW><<<grid, block>>>(                                          \
        ranges, point_list, W, H, means2D, cov3D_inv, projmatrix_inv, (float3*)cam_pos, colors, conic_opacity, \
        final_Ts, n_contrib, bg_color, pixel_colors, dL_dpixels, dL_dmean2D, dL_dconic2D, dL_dopacity, dL_dcolors)

    if (sort_settings.sort_mode == SortMode::GLOBAL)
    {
        renderCUDA<NUM_CHANNELS><<<grid, block>>>(ranges, point_list, W, H, bg_color, means2D, conic_opacity, colors,
                                                  final_Ts, n_contrib, dL_dpixels, dL_dmean2D, dL_dconic2D, dL_dopacity,
                                                  dL_dcolors);
    }
    else if (sort_settings.sort_mode == SortMode::PER_PIXEL_KBUFFER)
    {
        int window_size = sort_settings.queue_sizes.per_pixel;
        if (window_size <= 1)
            CALL_KBUFFER(1);
    ...
    else if (sort_settings.sort_mode == SortMode::PER_PIXEL_FULL)
    {
...
#define CALL_HIER(HIER_CULLING, MID_QUEUE_SIZE, HEAD_QUEUE_SIZE)                                                       \
    sortGaussiansRayHierarchicalCUDA_backward<NUM_CHANNELS, HEAD_QUEUE_SIZE, MID_QUEUE_SIZE, HIER_CULLING>             \
        <<<grid, {16, 4, 4}>>>(ranges, point_list, W, H, bg_color, means2D, cov3D_inv, projmatrix_inv,                 \
                               (float3*)cam_pos, conic_opacity, colors, final_Ts, n_contrib, pixel_colors, dL_dpixels, \
    ...
    {                                                                                      \
        ...
        {                                                                                  \
            throw std::runtime_error("Not supported head queue size " +                    \
                                     std::to_string(sort_settings.queue_sizes.per_pixel)); \
        }                                                                                  \
    }
    ...
    {                                                                                     \
        ...
        {                                                                                 \
            throw std::runtime_error("Not supported mid queue size " +                    \
                                     std::to_string(sort_settings.queue_sizes.tile_2x2)); \
        }                                                                                 \
    }
