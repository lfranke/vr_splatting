...
struct SortSettings
{
    ...
    {
        return sort_mode != SortMode::GLOBAL || sort_order == GlobalSortOrder::PER_TILE_DEPTH_CENTER ||
...
class Rasterizer
{
   public:
    ...
    static int forward(std::function<char*(size_t)> geometryBuffer, std::function<char*(size_t)> binningBuffer,
                       std::function<char*(size_t)> imageBuffer, const int P, int D, int M, const float* background,
                       const int width, int height, const SplattingSettings splatting_settings,
                       DebugVisualizationData& debugVisualization, const float* means3D, const float* shs,
                       const float* colors_precomp, const float* opacities, const float* scales,
                       const float scale_modifier, const float* rotations, const float* cov3D_precomp,
                       const float* viewmatrix, const float* projmatrix, const float* inv_viewprojmatrix,
                       const float* cam_pos, const float tan_fovx, float tan_fovy, const bool prefiltered,
                       float* out_color, int* radii = nullptr, bool debug = false, bool half_float = false);

    static void backward(const int P, int D, int M, int R, const float* background, const int width, int height,
                         const SortSettings sort_settings, const CullingSettings culling_settings,
                         const bool proper_ewa_scaling, const float* means3D, const float* shs, const float* opacities,
                         const float* colors_precomp, const float* scales, const float scale_modifier,
                         const float* rotations, const float* cov3D_precomp, const float* viewmatrix,
                         const float* projmatrix, const float* inv_viewprojmatrix, const float* cam_pos,
                         const float tan_fovx, float tan_fovy, const float* pixel_colors, const int* radii,
                         char* geom_buffer, char* binning_buffer, char* image_buffer, const float* dL_dpix,
                         float* dL_dmean2D, float* dL_dconic, float* dL_dopacity, float* dL_dcolor, float* dL_dmean3D,
                         float* dL_dcov3D, float* dL_dsh, float* dL_dscale, float* dL_drot, bool debug);
};
};  // namespace CudaRasterizer
