...
TEST(Loss, VGGCaffe)
{
    ...
    // auto loss_vgg = Saiga::PretrainedVGG19Loss(PROJECT_DIR.append("loss/vgg_script_caffe.pth").string());
    auto loss_vgg = std::make_shared<Saiga::PretrainedVGG19Loss>(
        Saiga::PretrainedVGG19Loss(PROJECT_DIR.append("loss/traced_caffe_vgg.pt").string()));
    loss_vgg->eval();
